...
// 验证层错误的计数
//
// 验证层报的 Error **从来不是故意的** —— 与应用自己打的 Error 不同 (判据会
// 故意制造溢出、故意让某一步失败, 那些 Error 是预期之内的)。所以它值得单独
// 计一个数, 并且这个数不为零就该让整次运行判失败。
//
// 在此之前, 被测那些帧里的验证层报错**一条都不影响退出码**: 自检的判定只看
// 各条判据自己的 passed, 而"关闭阶段的 Error"那一条是在全部自检跑完之后才
// 开始数的。也就是说渲染过程中的同步错误、布局错误、越界写, 只要没有哪条
// 判据恰好看得见, 就会一路绿着过去。
// ============================================================================

/// 到目前为止验证层报了多少条 Error
LIMX_NODISCARD UInt32 GetValidationErrorCount();
