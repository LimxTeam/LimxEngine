...
public:
    ...
    // 网格着色器绘制
    //
    // 参数是**工作组数**, 不是顶点数或图元数 —— 与 Dispatch 同构而与
    // Draw 不同构。名字里带 Tasks 是 Vulkan 的叫法, 它指的是任务着色器的
    // 工作组; 没有任务着色器时它就是网格着色器的工作组数。
    //
    // 设备不支持时**什么都不做并报错**, 不是静默返回。静默返回的后果是
    // 画面上少一整条渲染路径, 而那与"这条路径没启用"长得一样。
    // ====================================================================

    virtual void DrawMeshTasks(UInt32 groupCountX,
                               UInt32 groupCountY = 1,
                               UInt32 groupCountZ = 1) = 0;

    virtual void DrawMeshTasksIndirect(FRHIBufferHandle buffer,
                                       UInt64 offset,
                                       UInt32 drawCount,
                                       UInt32 stride) = 0;
