...
struct FTestRunOptions
{
    ...
    /// 期望至少执行这么多用例, 少于则判失败 (0 表示不检查)
    ///
    /// **这一条防的是"用例悄悄消失"。** 新加的套件只要没人往 verify.ps1 里
    /// 补一行, 就永远不会被执行 —— 而"没执行"与"全过"在退出码上完全一样。
    /// 已经这样漏过一批: ClusterGrid、Octahedral、ShadowAtlas 三个套件注册
    /// 了却从未在验证里跑过, 因为脚本是逐个套件名手写的。
    ///
    /// 判据放在**二进制里**而不是脚本里: 脚本要判断就得解析输出文本, 而
    /// 解析失败会退化成"没找到不一致"—— 又是一个落在"通过"上的失效。
    UInt32 MinTests = 0;
};
