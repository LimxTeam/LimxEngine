...
struct FSceneSyncStats
{
    ...
    /// 绘制时实际要绑定的材质描述符集次数
    ///
    /// bindless 下恒为 0 —— set 1 是全场景共享的一张表, 每段只绑一次,
    /// 逐 draw 只换 push constant 里的一个整数。
    ///
    /// 这个字段的语义是"绑定次数"而不是"材质变化次数"。两者在 bindless
    /// 之前是同一回事, 之后不是了 —— 继续用同一个数字会让人以为 bindless
    /// 没生效。
    UInt32 MaterialBindCount = 0;

    /// 相邻批次间材质下标发生变化的次数
    ///
    /// 与绑定次数不同, 这个数字仍然有意义: 它决定了将来 GPU 驱动渲染能把
    /// 多少相邻批次合并成一次间接绘制。排序做得越好这个数越小。
