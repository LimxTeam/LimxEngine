...
// ============================================================================
// LOD 选择 —— 与 Shaders/Builtin/meshlet_lod.h 逐字对应
//
// 两份实现必须给出**同一个**选中集。着色器那份参与渲染, 这份是判据的参考;
// 两者不一致时判据会报出来, 而那正是要它们分开写的理由。
// ============================================================================
...
///             "更早换到细的一层", 那是安全的一侧。
///
///   钳近平面: 不钳的话相机进球时 d -> 0, E -> +inf。两个 +inf 之间没有严格
///             大小关系, 而"不重不漏"的证明第二步正是要那个严格。近平面之内
///             本来就不画东西, 所以钳在这里不改变任何决策。
///
/// 保守性: 半径 e 的误差球在球心距 D 处的精确投影半径是 e/sqrt(D²-e²), 而
/// 这里的分母是 D-r。只有 r >= e 时 D-r <= sqrt(D²-e²), 结果才只会偏大 ——
/// 而 r >= e 是建 DAG 时钉死的不变量 (FLodGroup::Sphere 那段)。
LIMX_NODISCARD Float32 MeshLodProjectError(const FVector4& worldSphere,
                                           Float32         worldError,
...
///
/// 做错的两种典型形态与它们的画面表现:
///   * 父侧比较写成 > 而不是 >= —— 阈值恰好等于某个 E(g) 时父子都不选, 洞。
///     只在恰好相等时出现, 随机阈值几乎测不到。
///   * 自身球用 meshlet 自己的包围球而不是**产出组**的球 —— 兄弟拿到不同的
///     距离, 在同一阈值下一个换层一个不换, 而它们之间的边界在组内部没锁,
///     那是真裂缝。
LIMX_NODISCARD bool MeshLodSelect(const FLodMeshletRecord& record,
                                  const FVector3& cameraPosition,
                                  Float32 lodScale, Float32 nearPlane,
                                  Float32 threshold);
