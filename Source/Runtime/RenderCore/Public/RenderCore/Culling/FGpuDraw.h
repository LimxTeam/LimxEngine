...
/// 一帧最多参与 GPU 驱动绘制的物体数
///
/// 16384 × 96 字节 = 1.5 MiB, 每并行帧一份。固定上限而非动态扩容: 扩容意味着
/// 缓冲区换了句柄, 而描述符集指向的是旧句柄 —— 要么每帧重写描述符, 要么在
/// 某一帧读到已释放的显存。固定上限把这个状态整个消掉了。
///
/// 从 65536 降到 16384, 是因为间接命令缓冲区要按 **视图数 × 物体数** 开:
/// 相机一个视图, 三级级联三个。65536 那会儿是"只有相机一个视图"时随手取的
/// 数, 乘上视图之后就成了 8 × 65536 × 20 = 10 MiB 每帧。
///
/// 超过上限的物体退回逐物体绘制而不是丢弃 —— 丢弃的表现是"场景里少了一部分
/// 东西", 而那与资源加载失败长得一样。
inline constexpr UInt32 kMaxGpuDrawObjects = 16384;

/// 一帧最多剔除几个视图
///
/// 相机一个, 方向光三级级联三个, 剩下四个留给将来 (点光的立方体六面要占六个,
/// 那时这个数要提到 16)。
///
/// 每个视图在间接命令缓冲区里占 kMaxGpuDrawObjects 条命令的一段。段的起点是
/// 视图下标乘以那个数 —— **不是**乘以本帧的实际物体数。用实际数的话, 物体数
/// 一变每个视图的起点都跟着变, 而图形通道拿到的分组里存的是"本视图内的偏移",
/// 两处一旦不同步, 某个级联就会去画另一个级联的命令。
inline constexpr UInt32 kMaxCullViews = 8;
