...
//          BLAS 逐渲染对象建一个 (一个子网格一棵)，但 TLAS 里第 i 个实例
//          **不**一定对应对象列表里第 i 个 —— 没有三角形的对象会被跳过。
//          所以实例的 instanceCustomIndex 存的是**源对象下标**，几何表也
//          按源下标索引，两者由 m_SourceIndices 显式对上。
//          写成"第 i 个对第 i 个"的话，只要有一个对象被跳过，后面所有物体
//          的材质就整体错位一格，表现是"反射里的材质串了位" —— 而那是
//          几天后才会被发现的那种错。判据见 --rt-geometry-table-check：
//          它自己造一个被跳过的对象，因为真实场景里一个都没有。
