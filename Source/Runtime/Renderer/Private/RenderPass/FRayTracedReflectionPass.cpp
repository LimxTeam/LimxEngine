// ============================================================
// 文件名称：FRayTracedReflectionPass.cpp
// 创建时间：2026-08-31
// 创建者  ：LimxTeam
...
// 功能描述：FRayTracedReflectionPass 的实现 — 掩码纹理、计算管线、描述符与
//          每帧派发。
...
/// 反射图的格式
///
/// RGBA32F: rgb 是辐射度 (HDR, 可以超过 1), a 是命中距离。
///
/// 用全精度而不是 RGBA16F, 是因为 alpha 里装的是**判据要逐像素比对的
/// 命中距离** —— 半精度在十几个世界单位上只有三位有效数字, 那个精度
/// 与"实现错了"造成的偏差是同一量级。判据的分辨率不该由存储格式决定。
constexpr EPixelFormat kMaskFormat = EPixelFormat::RGBA32_SFLOAT;

/// 与 rt_reflection.comp 的 push constant 块逐字段一致
struct FRtReflectionPushConstants
{
    FMatrix InvViewProj;
    ...
    Float32 Ambient      = 0.03f;
};

static_assert(sizeof(FRtReflectionPushConstants) == 144,
              "FRtReflectionPushConstants 必须是 144 字节 — 与 "
              "rt_reflection.comp 的 push constant 块逐字段一致");
...
ERHIResult FRayTracedReflectionPass::Setup(const FPassSetupDesc& desc)
{
    ...
    LIMX_LOG(LogRenderer, Log,
             "[光追反射] 初始化完成 — {}x{} RGBA32F, 最大距离 {}",
             m_Extent.Width, m_Extent.Height, m_MaxDistance);
...
ERHIResult FRayTracedReflectionPass::CreateTarget(IRHIDevice* device,
                                             FRHIExtent2D extent)
{
    ...
    texDesc.DebugName   = "RayTracedReflection";

    ERHIResult result = device->CreateTexture(texDesc, m_ReflectionTexture);

    if (!IsRHISuccess(result))
    {
        LIMX_LOG(LogRenderer, Error, "[光追反射] 掩码纹理创建失败");
        return result;
    }

    FRHITextureViewDesc viewDesc = {};
    viewDesc.Texture         = m_ReflectionTexture;
    ...
    return device->CreateTextureView(viewDesc, m_ReflectionView);
}
...
ERHIResult FRayTracedReflectionPass::CreateDescriptors(IRHIDevice* device)
{
    ...
    layoutDesc.DebugName    = "RtReflectionSetLayout";
...
ERHIResult FRayTracedReflectionPass::CreatePipeline(IRHIDevice* device)
{
    ...
    ERHIResult result = shaders.CreateShaderModule(
        device, FString("Builtin/rt_reflection.comp"), EShaderStage::Compute,
        m_Shader);

    if (!IsRHISuccess(result))
    {
        LIMX_LOG(LogRenderer, Error, "[光追反射] rt_reflection.comp 加载失败");
        return result;
    }
    ...
    pushRange.Offset     = 0;
    pushRange.Size       = sizeof(FRtReflectionPushConstants);
    ...
    layoutDesc.DebugName              = "RtReflectionPipelineLayout";
    ...
    pipelineDesc.DebugName                = "RtReflectionPipeline";
...
void FRayTracedReflectionPass::Execute(IRHICommandBuffer*        commandBuffer,
                                    const FRenderPassContext& context)
{
    ...
    if (!m_LayoutInitialized && m_ReflectionTexture.IsValid())
    {
    ...
    if (!m_Tlas.IsValid() || !m_ReflectionView.IsValid() ||
        !m_DepthView.IsValid() || !m_GeometryTable.IsValid() ||
        !m_MaterialBuffer.IsValid())
    {
        return;
    }

    commandBuffer->BeginDebugLabel("RayTracedReflection", 0.6f, 0.4f, 0.9f);
    ...
    writes[3].ImageView     = m_ReflectionView;
    writes[3].ImageLayout   = EImageLayout::General;

    writes[4] = FRHIDescriptorWrite::StorageBuffer(
        m_DescriptorSet, 4, m_GeometryTable, 0, m_GeometryTableBytes);

    writes[5] = FRHIDescriptorWrite::StorageBuffer(
        m_DescriptorSet, 5, m_MaterialBuffer, 0, m_MaterialBufferBytes);
    ...
    // ---- 派发 ----
    FRtReflectionPushConstants push;
...
ERHIResult FRayTracedReflectionPass::OnResize(const FPassResizeDesc& desc)
{
    ...
    m_Device->DestroyTextureView(m_ReflectionView);
    m_Device->DestroyTexture(m_ReflectionTexture);
...
void FRayTracedReflectionPass::ReleaseSwapchainResources(IRHIDevice* device)
{
    if (device == nullptr)
    {
        return;
    ...
    device->DestroyTextureView(m_ReflectionView);
    device->DestroyTexture(m_ReflectionTexture);
}

void FRayTracedReflectionPass::Shutdown(IRHIDevice* device)
{
    if (device == nullptr)
    {
        return;
