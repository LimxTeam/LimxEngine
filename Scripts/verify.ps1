...
# meshlet 分组与组间边界锁定 —— 纯 CPU。
#
# 虚拟几何"无裂缝"的地基。最要紧的一条: 相邻两组各自简化之后, 共享边界上的
# 顶点位置**逐位相同**。LOD 要能逐块选层 (近处高模、远处低模), 那就要求每块
# 能独立简化; 两块各自简化之后边界如果各自动了, 画面上就是一条能看见背景的缝。
#
# 另外六条: 划分性质、组大小、组内连通、边界判定 (判据用另一套算法重算一遍)、
# 组数不许碎、分组质量。
#
# 最后一条是防"什么都不做也满分"的: 一个完全不看邻接关系的分组照样是合法划分,
# 上面每一条都过。所以要求贪心必须打赢**按下标顺序**这个免费基线 (meshlet 切分器
# 本身已按邻接聚类, 顺序切出来的组相当好), 并且明显好于打乱后的分法。
# 实测 76 / 136 / 208。
#
# 变异 4/7 (三条逃逸的是"质量退化但仍在及格线以上", 见提交说明)。
Invoke-Step 'meshlet 分组 (组间边界逐位不动)' {
    Invoke-Engine '--frames 1 --warmup 0 --meshlet-group-check'
}

# LOD DAG 构建 —— 纯 CPU。
#
# 反复"分组 -> 组内简化到一半 -> 按组重新切 meshlet"。DAG 的边是**组**级的:
# 一个组被当作整体简化, 所以父子关系是"组的全部成员 <-> 组产出的全部 meshlet"。
# 记成"重叠最大的那一个 meshlet"的话, 同组的两个兄弟会拿到不同的父误差, 于是
# 在同一个相机下一个换层一个不换 —— 而它们之间的边界在组**内部**, 没被锁定,
# 那是真裂缝。
#
# 八条判据里最要紧的是**误差必须是相对原始表面的上界, 逐层成立**。它必须对
# 原始网格量而不是对上一层量: "误差取 max 而不是累加"那个错在对上一层量时是
# 绿的, 只有对原始网格量才红。而那个错的后果是"误差是上界"变成假话 —— 画面上
# 远处轮廓走形、细节被抹平, 别的判据全绿。
#
# 其余七条: 叶子层与原始三角形集合逐个相同 (连绕序)、逐层减半、逐 meshlet
# 单调 (父误差**严格**大于子, 且父球含子球, 且半径不小于误差)、拓扑排序无环
# (只用父子边, 不信层号)、每层各自是划分、组的边界顶点在父层里逐位存在、
# 有父的组必须真的减少了三角形。
#
# 实测: 球体 6 层 6016->3004->1500->750->374->240, 误差 0.0093/0.0183/0.0468/
# 0.126/0.641 而报出 0.0093/0.0243/0.0684/0.193/0.832 —— 每层都是上界。
#
# 变异 3/8 (四条逃逸都是"被更强的守卫盖住"或测度为零, 原因已逐条测出并记在
# 变异脚本里)。
Invoke-Step 'LOD DAG (误差逐层是原始表面的上界)' {
    Invoke-Engine '--frames 1 --warmup 0 --lod-dag-check'
}

# LOD 选择规则 —— 纯 CPU。
#
# 规则是 "自身误差 < 阈值 **且** 父误差 >= 阈值"。三层判据:
#
#   边判据。屏幕误差沿 DAG 的每条边严格增, 对每一个探针相机。加上两端哨兵
#     (叶子 0、根 1e30), 表面上任意一点诱导出的链就是严格递增、从 0 到 +inf,
#     而有限的正阈值恰好落进其中一段 —— 存在则不漏, 唯一则不重。验完全部边
#     就等于验完全部点, 不必枚举路径。
#     探针相机必须含退化位姿: 在某个 LOD 球**内部**、贴近平面、极远、侧向
#     偏心。正对着看是最容易过的那一种。
#
#   覆盖。边判据是靠证明推出割性质的, 而证明本身可能哪里没想到。这一条不推,
#     直接数: 表面上撒点, 每个点在每一层找到罩着它的 meshlet, 数有几个被选中,
#     必须恰好 1。实测 18060 个 (相机, 阈值, 采样点) 全部命中一次。
#
#   元判据。选中的三角形数必须随相机拉远单调不增, 而且真的在变。没有这一条,
#     一个"永远选叶子层"的实现在前两条上满分通过 —— 它确实不重不漏。
#     实测距离 1.5/3/6/12/25/60 上是 6016/6016/3724/3004/1500/750。
#
# 阈值集合里放了"恰好等于某个 E(g)"的值 —— 父侧比较写成 > 而不是 >= 时,
# 那个阈值上父子**都不选**, 一个洞, 而随机阈值几乎测不到。
#
# 变异 6/7。逃逸那条 (距离用球心而不是球面最近点) 是真缺陷, 但它对自身与父
# 一视同仁, 单调性与割性质都保住了 —— 要抓它得把屏幕误差与实际像素偏差挂钩。
Invoke-Step 'LOD 选择 (恰好命中一次)' {
    Invoke-Engine '--frames 1 --warmup 0 --lod-select-check'
}

# LOD 无裂缝 + 屏幕误差的承诺 —— 纯 CPU。
#
# 两条判据, 各自钉住一件此前没人验过的事。
#
#   **选中集是闭合流形。** 这是"无裂缝"最锋利的形式: 选中集跨越多个层 ——
#     近处一块用叶子层、远处一块用第四层 —— 而缝在这里的表现极其直接: 一条
#     只被**一个**三角形用到的边。
#     它比"逐像素看有没有洞"强得多: 后者依赖分辨率与视角, 一条亚像素的缝在
#     1280x720 上看不见, 换个分辨率就露出来; 而边的计数与两者都无关。
#     元判据: 跨两层以上的情形必须够多 (绝对数 >= 5)。全是单层的话这条验的
#     只是"某一层自己是闭合的", 那是 DAG 判据早就验过的事。
#
#   **屏幕误差的承诺必须兑现, 逐 meshlet。** 选择规则保证
#     自身误差·lodScale/(球心距-半径) < 阈值, 而 meshlet 的几何全在那个球里,
#     所以拿它到相机的**真实最近距离**投出来只会更小 —— 这是可证的, 所以判据
#     **不留任何容差**。
#     必须逐 meshlet 而不是逐采样点: 按采样点量会被稀释 (相机近处恰好选细层,
#     偏差近于零), 第四天逃掉的那条变异正是这么逃的 —— 它把逐采样点的最坏值
#     从 0.414 推到 0.972 倍阈值, 量到了劣化却没破约。换成逐 meshlet, 同一条
#     变异是 111 次破约、最坏 6.3 倍。
#
# 变异 5/7。两条逃逸原因已测: `>` vs `>=` 要靠"阈值恰好等于某个 E(g)"才显形
# (--lod-select-check 里有那种阈值, 在那边是红的); "不锁组间边界"被
# LockOpenBoundary 盖住 —— 同一次扫描里把 LockOpenBoundary 关掉是红的。
Invoke-Step 'LOD 无裂缝 (选中集是闭合流形)' {
    Invoke-Engine '--frames 1 --warmup 0 --lod-crack-check'
}

# LOD 选择的 GPU 实现 —— 需要真实 GPU。
#
# 参与渲染的是着色器那一份 (Shaders/Builtin/meshlet_lod.h), 而前面五天全部
# 判据验的是 C++ 那一份。两者不一致的话, 那五天的结论对画面**一句都不成立**。
#
# 判据把两份实现的中间量 (自身屏幕误差、父屏幕误差) 逐条比。阈值集合里放了
# "恰好等于某条记录屏幕误差"的值 —— `<` 与 `<=`、`>=` 与 `>` 之别唯一显形的
# 地方。
#
# ── 关于"逐位相同" ──
#
# 第一版要求逐位, 当场红了: 自身 1480 次、父 3240 次不同, 最大绝对差 2.13e-4。
# 而那不是缺陷 —— **Vulkan 不保证与 CPU 逐位相同** (规范允许 sqrt 有 3 ULP,
# 除法 2.5 ULP)。要求逐位就是要求平台没承诺的东西。
#
# 真正要保住的是: 选择规则只依赖"屏幕误差沿 DAG 每条边**严格增**", 而建 DAG
# 时的增长地板是 2^-10 的相对增长 —— 只要两份实现的相对差远小于它, 那个严格序
# 在 GPU 上照样成立。所以预算取地板的百分之一, 它不是拍脑袋的容差, 是"离破坏
# 不变量还差两个数量级"。实测最大相对差是预算的 **0.068 倍**。
#
# 还有一条元判据: 相对差不许**恰好为零** —— Vulkan 既然不保证逐位, 全零更像是
# 探针没跑起来或回读了一片零。这个项目栽过一次"恒为零的诊断量"。
#
# 变异 6/6 (全部打在着色器那一份上, C++ 参考不动 —— 这样验的正是"两份是否
# 一致", 而不是"某一份是否自洽")。
Invoke-Step 'LOD 选择的 GPU 实现 (与 CPU 参考一致)' -RequiresGpu {
    Invoke-Engine '--frames 2 --warmup 1 --lod-gpu-check'
}

# LOD DAG 构建的规模性质 —— 纯 CPU。
#
# 这一条不验几何, 验的是**能不能用**。接资源管线的前提是"能在可接受的时间里
# 把 DAG 建出来", 而那种瓶颈不会让任何几何判据变红 —— 只会让导入从一秒变成
# 一分钟, 直到有人受不了去关掉 LOD。
#
# 判的两件事:
#
#   **局部阶必须 <= 1.5。** 这个算法应当线性: 简化器的收尾是 O(点数 ×
#     三角形数), 但它跑在**组**上 (每组至多 32 个 meshlet), 而组数随规模线性
#     增长。超线性说明"组的规模有界"这条塌了 —— 例如分组退化成一个巨大的组。
#     实测五档: 局部阶 0.91 / 0.79 / 1.05 / 0.84, 吞吐 43k-58k 三角形/秒。
#
#   **每一档的不变式仍然成立。** 单调、父球含子球、叶子层无损。规模上去之后
#     不变式塌掉很常见 (分组更碎、简化更容易停滞), 不会自动跟着成立。
#
# 还有一个**不判定但必须印**的数: 吞吐外推到 Sponza (262000 三角形) 约 4585 ms,
# 而导入本身的中位数是 1250 ms —— 也就是说 DAG 若在**导入时**建, 导入会变成
# 原来的 4.67 倍。结论是它属于**烘焙期**而不是加载期, 而这个结论的全部依据
# 就是这个数。
#
# 变异 3/4 (逃逸的那条与第三天同因: 增长地板防的是测度为零的情形)。
Invoke-Step 'LOD DAG 规模 (构建必须线性)' {
    Invoke-Engine '--frames 1 --warmup 0 --lod-scale-check'
}

# 采样器与多重重要性采样 —— 需要真实 GPU (探针是 compute)。
#
# 第八天的白炉与梯度炉验的是"合起来那个数对不对" (间接、端到端)。这一条验的是
# **分布本身**, 而且对得上的是解析值:
#
#     E[cosθ] = 2/3    E[cos²θ] = 1/2    E[cos³θ] = 2/5    E[方向] = (0,0,2/3)
#
# 两条都要: 炉子抓得住"采样与 pdf 不匹配", 但抓不住"分布对、pdf 也对, 只是方位角
# 有偏" —— 一阶矩的横向分量会把它顶出来, 而各向同性的炉子把它在球面上平均掉了。
#
# 还有两条逐样本的恒等式 (不是统计量, 所以预算只留浮点舍入):
#   * pdf·π/cosθ 恒为 1 —— 抓"采样函数改了而 pdf 没跟着改"
#   * 采出来的方向必须是单位向量 —— 这是**唯一**看得见 r 那一支的量。矩全都
#     只用 cosθ, 横向分量又平均为零, 所以 "r = u1 而不是 sqrt(u1)" 曾整个逃掉。
#
# MIS 那两条更硬: 权重之和必须恒为 1 (代数恒等式), 那是无偏性的全部依据。
# 判据造的 pdf 跨到 e^±45 —— 平方之后 1.2e39, **溢出 float32**。第一版取 e^±15,
# 平方才 1e13, 于是"幂启发式不先归一化"整个逃掉了。
#
# 误差预算用**解析**方差而不是实测: 实测的 σ 本身受被验对象影响, 采样器错了 σ
# 也跟着错, 预算会跟着变宽 —— 那正是"判据的失败模式落在通过上"。
#
# 变异 6/6。
Invoke-Step '采样器与 MIS (对得上解析值)' -RequiresGpu {
    Invoke-Engine '--frames 2 --warmup 1 --sampler-check'
}
...
# —— 那不是自动成立的。种子在帧间卡住的话:
#   * 均值仍是那一个样本的值 (有偏, 但看起来"就是有点噪")
#   * 方差**完全不下降** —— 画面永远那么噪, 而人会以为"再等等就好了"
#   * 每一帧逐位相同, 画面看起来反而更**稳** —— 而稳正是时域累积想要的效果
# 最后那一点最阴, 所以这条判据不看画面, 看方差。
#
# 四条: N 帧 x 1 spp 与 1 帧 x N spp 的均值一致 (实测偏差 2.0e-4, 预算 2.4e-3);
# 两者的单样本方差之比必须接近 1 (实测 1.0035 —— 这一条抓种子卡住);
# 累积帧数恰好是 N; 分块派发的每一块贡献都要留下。
#
# 最后那条补的是一处**全仓的覆盖空白**: 追踪器把 spp 分块派发 (每块 4M 条
# 路径), 而现有判据的 spp 全都落在一块之内 —— 96x96 上每块是 455 spp, 而最多
# 只跑 256。也就是说"分块"那段代码此前一个判据都没有。这里用 256x256 跑 192 spp
# 逼出三块, 并且**两个槽位都验**: 命中计数在槽位 1、辐射度在槽位 0, 只改一个
# 的话另一条看不见 (第一版正是只验了计数, 变异照样逃)。
#
# 变异 4/5。逃逸那条是结构性的: 这条判据比的是同一个着色器的两条路径, 任何
# **共同**的偏差都被约掉 —— 漏掉反照率由第八天的能量守恒抓。
Invoke-Step '时域累积 (跨帧样本必须独立)' -RequiresGpu {
    Invoke-Engine '--frames 2 --warmup 1 --gi-accum-check'
}

# 离线参考路径追踪器 —— 需要真实 GPU。
#
# 这一条验的不是画面, 是**后面所有实时 GI 的参考答案本身对不对**。三条判据:
#
#   白炉。纯白环境 (辐射度 1) + 纯白 Lambert (反照率 1), 收敛后必须恰好是
#     1.0。凸球上每个样本恰好是 1.0, 单样本方差为零, 所以这一条的统计预算
#     是**零**, 容差只剩浮点舍入 (1e-6)。它对"看起来对"完全无感。
#     白炉单独验不了采样分布 —— 各向同性环境下余弦加权平均与均匀加权平均
#     是同一个数。所以配了一个带梯度的环境 L(ω) = 1 + ω.y, 水平面上解析值
#     5/3, 而"按均匀采样却按余弦 pdf 算权重"给出 3/2, 差 10%。
#     再配一个开口 Cornell 盒验多次弹射: 反照率为 1 时截断偏差恰好等于存活
#     比例, 于是有精确恒等式 L + 截断 ≡ 1 可判 (实测残差 0)。
#
#   能量守恒。封闭自发光盒 (L_e = 1, 反照率 a) 里辐射度处处是 1/(1-a),
#     截到 N 次弹射是 (1-a^(N+1))/(1-a) —— 逐个 N 都有解析值。四个反照率
#     × 七个弹射次数, 全部落在 5.5σ/√n 之内。
#
#   方差 ∝ 1/N。四个 spp 点各跑 12 次独立试验, 对数坐标上回归, 斜率必须
#     接近 -1。这一条验的是"样本之间真的独立" —— 前两条都验不到。
#
# 误差预算一律由**实测的**单样本标准差推出: 5.5σ/√N。5.5 是 Bonferroni 的
# 账 (16384 个像素, 双侧误判率 3.8e-8 × 16384 = 6.2e-4), 不是拍脑袋的余量。
# 追踪器完全确定性 (种子是 (像素, 样本号, 试验号) 的哈希), 同一台机器上
# 逐位可复现, 所以这条判据没有抖动。
#
# 变异 8/9。逃掉的那条是"命中点用 origin+t·dir 而不是重心坐标重建" ——
# 判据的场景只有两个世界单位大, t 不超过 4, 而法线偏移 1e-4, 两种重建差
# 3e-6, 远在任何预算之下。这一条的覆盖边界在**尺度**上, 记下来。
Invoke-Step '离线参考路径追踪 (白炉 + 能量守恒 + 方差标度)' -RequiresGpu {
    Invoke-Engine '--frames 2 --warmup 1 --path-trace-check'
}
...
# 规模: 展开顶点流溢出之后不许照着计数器绘制 —— 需要真实 GPU。
#
# 与上面那条 "GPU 剔除溢出"是**同一类**缺陷的另一处: 容量被突破之后, 消费的
# 一侧照着计数器走, 而不是照着真正写进去的数走。
#
# 回退路径 (计算展开 + 经典顶点着色器) 把可见 meshlet 展开成一条定容的顶点
# 流。展开着色器里那次原子加是槽位分配器, 领到的位置超出容量的三角形被丢掉。
# 而那个计数器同时又是 vkCmdDrawIndirect 读的 vertexCount —— 被丢掉的三角形
# 照样把它加了上去, 于是命令处理器按一个**大于流里真实顶点数**的值发顶点,
# meshlet_depth_fallback.vert 又拿 gl_VertexIndex 无条件索引那条流。
#
# 后果不是"画面上多一块垃圾", 是 GPU 读缓冲区之外的地址、设备丢失。
#
# 判据盯不变式: **交给 DrawIndirect 的顶点数 <= 流的容量**, 而且恰好等于那个
# 连续前缀的长度 (多了是越界, 少了是白丢)。与堆布局无关 —— "会不会丢设备"
# 取决于越界读到的那页映没映射, 拿它当判据是让判据去赌运气。
#
# 判据自己造条件: 流按两百万个顶点开的, 靠场景规模撑满要几十万个可见 meshlet。
# 所以它把容量压到四分之一逼那条路径走一遍, 再验三段 —— 正常容量不报溢出、
# 压小容量必须报且交出去的数不许越界、恢复容量之后标志不粘着。
#
# 变异 3/3 (计数器加在判越界之前 / 只用一个计数器 / 溢出时把计数器夹到容量)。
Invoke-Step 'Meshlet 展开溢出 (交给 DrawIndirect 的顶点数不许越界)' -RequiresGpu {
    Invoke-Engine '--grid 24 --frames 4 --warmup 2 --meshlet-expand-overflow-check'
}
