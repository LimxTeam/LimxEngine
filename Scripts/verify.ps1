...
# **加速结构里装的是不是屏幕上那个场景**: 真实网格的顶点跨度、子网格的
# 索引偏移、每个物体的变换、以及渲染对象列表与 BLAS 的对应关系。
#
# 光栅器与 BVH 是两条彻底独立的路径 —— 一条按三角形扫描填深度, 一条按射线
# 遍历树。它们对同一个像素给出同一个深度, 才说明两边看的是同一个场景。
#
# 跑在阴影场景上 (三个物体, 各有自己的平移与缩放, 几何体填满整屏):
#   921600 / 921600 个像素一致, 最大相对误差 1.1e-5。
#
# 已知的覆盖边界: 这个场景里几何体填满了整屏, 所以判据只验了"命中处的深度",
# 没验"该落空的地方要落空"。综合场景 (33 个物体, 有天空背景) 目前**过不去**
# —— 背景像素的深度回读出来是 NDC 0 而不是清除值 1.0, 而同一帧的画面与基线
# 一致。根因未定, 记在 RunRayTracingDepthCheck 的注释里。判据不因此放宽。
Invoke-Step '光追深度 (与光栅化深度逐像素比对)' -RequiresGpu {
    Invoke-Engine '--shadow-scene --shadow-lights 2 --clustered --frames 8 --warmup 3 --rt-depth-check'
}
