...
/// 深度 + UV → 视空间位置
///
/// 走逆投影矩阵, 每样本 16 次乘加加一次除法。
///
/// **试过更便宜的写法, 实测没有收益。** 透视投影下视空间位置可以写成
/// "视线方向 x 线性深度", 每样本只要 3 次乘法。改完之后逐像素结果完全
/// 一致 (墙角处 0.585693 vs 0.585694), 而 GPU 耗时是 0.856 ms 对 0.854 ms
/// —— 各测三次取中位数, 差异落在噪声里。
///
/// 原因是这个着色器**不是 ALU 受限的**: 每像素 64 次深度采样, 取数延迟
/// 完全盖过了算术。真正的杠杆是减少采样次数 (半分辨率 + 双边上采样),
/// 而不是让每次采样更便宜。
///
/// 这条注释留着, 是为了下一个想优化这里的人不必再量一遍。
