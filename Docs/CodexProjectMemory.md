...
把 meshlet 路径搬到 1601 个物体的压力网格上跑一遍, 立刻掉出一个从第八天
就存在的缺陷。

### 一致性判据只验转写

法线锥存进去的是半角**余弦**, 而背面剔除要拿来比的是半角**正弦**。

推导一句话: 锥里每条法线满足 dot(n, axis) >= mindp, 半角 α = acos(mindp);
设视线与轴的夹角为 θ, 锥内法线在视线上投影的最小值是 cos(θ + α)。"整个
meshlet 都背对着"要求它仍为正 —— 即 θ + α < 90 度, 即 **cos θ > sin α**。

    α < 45 度   cos α > sin α   门槛偏高 -> **漏剔** (保守, 画面全对)
    α > 45 度   cos α < sin α   门槛偏低 -> **错剔** (画面上少一块)

综合场景里的球是 64x48 段的, 每个 meshlet 跨的曲率小, 半角一直在 45 度
以内 —— 于是它一直落在漏剔那一侧。Day 8 的构建器判据、Day 9 的剔除判据、
Day 10 的两条路径逐位相同、Day 12 的解析判据, **从头到尾全绿**。

压力场景里的球只有 16x12 段。半角越过 45 度, meshlet 路径与经典深度立刻
差 1490 个像素, 而且全部是 meshlet 路径**更远** —— 它把看得见的东西剔掉了。

Day 9 的判据为什么没抓住: 那条判据比的是"GPU 与 CPU 参考实现逐条相同",
而 CPU 参考照着同一个公式写。**两份同样的错互相印证不出任何东西。**

一致性判据验的是转写 —— 从公式到 GPU 代码、从公式到 CPU 代码这两条路上有
没有抄错。公式本身对不对, 它一个字都没说。

### 地面真值: 不走公式

补的判据不看锥: 对每一个被背面剔除剔掉的 meshlet, 把它的每个三角形变换到
世界空间, 逐个验它确实背对着相机。这是"背面剔除"四个字的定义, 与用什么锥、
锥怎么存、锥怎么算都无关。

它在**综合场景**上立刻报了:

    背面剔除剔掉 18 个 meshlet, 其中 15 个里还有正对着相机的三角形 (共 245 个)

那个缺陷一直在那儿。它没在画面上露头, 是因为那些 meshlet 恰好被别的几何体
挡着 —— 剔掉与不剔掉画出来一样。**后果不在画面上**, 那是四个覆盖边界里的
第三个。
...
"正对着相机"第一版按 Vulkan 的定义算: 投到裁剪空间, 取归一化设备坐标里的
有符号面积, 管线声明 FrontFace::CounterClockwise 就取面积为正。

它与最终版的结论**恰好相反** —— 6450 个三角形里只有 8 个一致, 而那 8 个是
退化的。差在投影矩阵翻不翻 Y: 翻了的话归一化设备坐标里的绕序与世界空间
反过来。从"管线声明了什么"推回世界空间, 要先知道这一点, 而那是猜。

改成按外法线判, 并且外法线的方向**是量出来的**: 叉积 cross(b-a, c-a) 与
顶点里的作者法线同向 6450/6450, 每次运行都数一遍。这个数既是那句话的证据,
也顺带验了取几何的下标 —— 下标错了取出来的是别的三角形, 同向率会立刻掉。

再往前一版更糟: 在留下来的 meshlet 上数两种约定各有多少"正面", 取多的那个。
结果是 2897 比 3553 —— 接近五五开, 因为封闭物体本来就有一半三角形背对着
相机。**那不是判据, 那是抛硬币。**

### 装不下的时候要响

可见表与待定表都是定容的, 着色器里超出容量的条目直接丢掉。丢掉本身没得选
(总不能越界写), 问题在于第一版**一个字都不说**: 画面上少一块, 日志里干干
净净, 而且只在场景大到一定程度才出现。

计数器本来就记着这件事 —— atomicAdd 照加不误, 超出容量的那几条只是不写,
于是"要写的条数"大于容量就是溢出的证据。只是没人读它。

判据自己造条件 (靠场景规模走不到: 可见表按 262144 条开的), 把容量压到可见
数的四分之一, 逼那条路径走一遍, 验三件事:

    报得出来
    写进去的那部分仍然**自洽** —— meshlet 必须落在自己实例的区间里
    容量恢复之后标志不粘着

第二条不是"各自在范围内": 一条记录是两个数, 两个线程抢同一个槽位时会拼出
"甲的实例 + 乙的 meshlet", 而那两个数各自都合法。

第三条要紧: 一个"一旦置位就再也不清"的标志在前两段上满分通过, 而它会让
之后每一帧都报溢出。

### 又一个恒为零的字段

统计里的 InstancesVisible **从来没被赋过值**。没有人看它, 于是它安安静静地
报了不知道多少次"第一级把所有实例都剔光了"。补上从间接分派参数的回读之后
是 951/1601, 而且与第二级的数对得上。

这是本周期第三次撞上同一件事 (Day 13 的第二阶段计数恒为零、那一天的两条
死分支)。**一个恒为零的诊断量比没有诊断量更糟** —— 它看起来像信息。

### 字段的含义变了, 判据把它顶了出来

法线锥从存余弦改成存正弦之后, Day 8 的构建器判据立刻报了十几条 ("锥余弦
落在 (-1, 0] 里"、"三角形的法线落在自己的锥外")。

那正是它该做的。**字段的含义变了而读它的地方没跟上**, 判据把这件事顶出来,
而不是默默接受一个换了单位的数。

### 一万六千个物体上的设备丢失 —— 查到底了

第十四天只查到"是 CPU 侧的上传写触发的"就停了。第十五天开了
VK_EXT_device_fault, 一句话到底。

**根因与 meshlet 无关。** 逐物体缓冲区 (模型矩阵 + 材质下标) 定容 16384,
分三段: 相机 / 投射体 / 半透明。三段合计超容量时后面的段被截断 —— 截断本身
没得选, 要命的是**绘制那一侧照着列表长度走**: 各 Pass 把列表下标当
firstInstance 传进去, 而 pbr.vert 拿 gl_InstanceIndex 直接索引那个缓冲区,
没有任何边界。列表比写进去的条目长时, 后面那些物体索引到缓冲区之外, 读出来
的"材质下标"是垃圾, 下一步要去索引 bindless 材质表。

    grid 126   15877 个物体   截断没落在投射体段   十次全过
    grid 127   16130 个物体   截断落在投射体段     每次第三帧丢设备

修法: 没有逐物体条目的物体**不画**。三条路径各加一次判断 (级联阴影、阴影
图集、前向半透明), 并把半透明段实际写进去的条目数暴露出来 —— 原来只有段
起点, 没有段长度。修完 grid 127 / 128 / 200 (四万个物体) 全都干干净净。

### 为什么二分查不出来, 而一个扩展一句话说清

第十四天的二分给出的是一组**互相矛盾**的结论:
...
按这组结论根本拼不出一个说得通的故事 —— 因为它们全是**间接**的: meshlet
通道的存在只是改变了堆布局与内存内容, 而真正越界的那次读发生在别的通道,
读到的是缓冲区之后那一页里恰好躺着的东西。写进去什么、写多少, 决定了那一页
里是垃圾还是零, 也就决定了那次越界读会不会踩到没映射的地方。

VK_EXT_device_fault 直接把答案摆出来:

    地址 0 — 类型 6 (指令指针故障)
    地址 1 — 类型 1 (**非法读**) 报告地址 2730557440 精度 4096

把当时所有活着的缓冲区的设备地址打出来一比: 2730557440 **不属于任何一个**。
一个不属于任何资源的地址被读了 —— 那就只能是索引跑到了缓冲区之外。顺着
"哪来的野指针"十分钟到底。

这个扩展留下了, 写成正式功能: 以后任何一次 VK_ERROR_DEVICE_LOST 都会带上
故障地址与读写类型。(踩了一脚: 只加扩展不够, **特性**也要在 pNext 里开,
否则 vkGetDeviceFaultInfoEXT 一调就把进程带走。)

**教训**: 二分对"症状与触发点隔着几层"的缺陷几乎无效。它给出的每一条结论
单独看都对, 拼起来却互相矛盾 —— 而那个矛盾本身就是信号: 说明被二分的那个
量不是因果链上的一环。这种时候该做的不是继续二分, 是去找一个能**直接**
问出真相的工具。

### 判据盯不变式, 不盯症状

--gpu-cull-overflow-check (退出码 35) 的判据换过两版才对:

    第一版   "跑完不许丢设备"
             不行。越界读到的内存是不是已映射取决于堆布局 —— 实测把钳位
             删掉之后, 同一个场景有时丢设备有时安然无恙。拿它当判据等于
             让判据去赌运气。

    第二版   "跳过的绘制数 == 列表长度减写入数"
             也不行。各 Pass 每帧尝试画多少个与级联、光源、逐通道剔除都
             有关, 预测不了。实测 877 vs 1324, 而 877 是对的。

    第三版   "**发出去的最大逐物体下标必须小于写进去的条目数**"
             不用预测, 与堆布局无关, 就是那条不变式本身。

压力场景补了两样东西, 都是变异验证逼出来的: 八个半透明物体 (半透明是最后
一段, 不放东西的话"半透明段被挤没之后照画"这条路径永远走不到) 和一盏投影
聚光灯 (原本只有方向光, 阴影图集那一路从来没被走到)。变异 5/5。

### 变异 7/8

    法线锥存半角余弦而不是正弦        红 (地面真值判据)
    法线锥存半角余弦 (构建器判据)      红
    法线锥存半角余弦 (压力场景画面)     红
    背面判据拿余弦比而不是正弦        红
    容量溢出不报                 红
...
最后一条的逃逸原因是量出来的, 不是猜的: 绕回去 (slot % capacity) 写出来的
仍是一批合法条目, 只是换了一批 —— 两种做法都是"丢掉一部分", 谁也不比谁更
正确; 而它唯一的真实风险 (两个线程抢同一槽位拼出撕开的记录) 在本机驱动上
不成立, uvec2 是一次八字节写。

## 第六个周期结束时的状态

**硬件光追** (Day 1–7): RHI 层的加速结构抽象 / BLAS 与 TLAS 构建与刷新 /
ray query 内联光追 / 光追阴影 / 光追环境光遮蔽 / 光追反射 / 与光栅路径的
混合策略。

**虚拟几何** (Day 8–14): meshlet 切分 (64 顶点 / 124 三角形) / 两级 GPU
剔除 (实例 + meshlet, 视锥 + 法线锥) / 网格着色器路径与计算展开回退 /
可见性缓冲 / 材质解析 / Hi-Z 两阶段遮挡剔除 / 规模压力测试。

**验证**: verify.ps1 65 步全绿。第六周期新增的每一天都做了变异验证:

    Day 8  meshlet 切分        11/11
    Day 9  两级剔除             8/8
    Day 10 网格着色器与回退      11/11
    Day 11 可见性缓冲            8/8
    Day 12 材质解析             11/11
    Day 13 Hi-Z 两阶段遮挡       11/11 (第 12 条构造上不可达)
    Day 14 规模                  7/8  (第 8 条的逃逸原因已测)
    Day 15 逐物体缓冲区截断        5/5
...
第五周期的教训是"判据失败时要能被看见"。这个周期往前走了一步: 有些东西
**判据的形状本身就不对**。

两阶段遮挡剔除是自纠错的 —— 第一阶段错剔了, 第二阶段拿重建后的金字塔一测
就补回来了。于是"开关遮挡剔除画面必须相同"这条判据对遮挡测试本身的错误
几乎是瞎的: 十一条变异只红了三条, 剩下八条损害的是**效率**而不是正确性。

那八条不是判据不够严, 是它**问错了问题**。补的办法是越过画面, 直接给那两个
函数写一层探针判据, 逐个验四条单向不等式。加上之后是 11/11, 而且当场量出
两个真缺陷 (四角近似包不住透视投影的球、"离相机最近的点"不是深度最小的点),
修完第二阶段的补画工作量少了 46%。

**二、一致性判据只验转写, 不验公式。**

Day 9 拿 CPU 参考实现逐条比 GPU 的剔除结果, 8/8 全红, 看起来很扎实。但法线锥
存进去的是半角余弦而背面剔除要拿半角正弦比 —— 这个错从第八天一路绿到第
十四天, 因为 **CPU 参考照着同一个公式写**。两份同样的错互相印证不出任何东西。

抓到它的是两件事: 换一个几何更粗的场景 (半角越过 45 度, 那个错从"漏剔"翻成
"错剔"), 以及一条不走公式的地面真值判据 (被剔掉的 meshlet 里不许有正对着
相机的三角形)。后者在**综合场景**上也立刻报了 245 个 —— 缺陷一直在, 只是
那些 meshlet 恰好被挡着, 后果没落到画面上。

**三、不会红的分支就是没有判据的分支。**

这个周期删掉或标注了五处构造上永远不会红的判断:

    相机在球内的早退           被近平面判断完全盖住
    近平面判断写了两遍          两句是同一个条件
    第二阶段的投影判断          待定表里的记录都是投影成功才追加的
    第二阶段补回来的计数         拿剔除通道的统计算, 恒为零
    可见实例数                 从来没被赋过值, 恒为零

后两条尤其值得记: **一个恒为零的诊断量比没有诊断量更糟**, 它看起来像信息。
第二阶段那个恒为零的计数掩盖了另一个缺陷 (分派参数的 y/z 没置 1, 整个第二
阶段是死代码), 而判据当时全绿。

### 留着的那一条, 收尾时查掉了

一万六千个物体上的设备丢失 —— 根因是逐物体缓冲区被截断而绘制仍按列表长度
索引, 与虚拟几何无关 (meshlet 通道只是改变了堆布局, 让越界读踩到了没映射
的页)。详见 Day 14 那一节。

它留下的一条方法论比修复本身值钱: **二分给出互相矛盾的结论时, 矛盾本身就
是信号** —— 说明被二分的那个量不在因果链上。那时该换工具, 不该继续二分。
VK_EXT_device_fault 一句话说清了二分半天没说清的事。


# 第七周期 — 虚拟几何的 LOD 与全局光照

上个周期的 meshlet 是**单层**的: 有切分、有两级剔除、有可见性缓冲, 但没有
LOD。这一半是虚拟几何真正难的地方。

    Day 1–7    虚拟几何的 LOD (简化 -> 分组 -> DAG -> 选择 -> 无裂缝)
    Day 8–14   全局光照 (参考路径追踪器当 oracle -> 实时 GI)
    Day 15     收尾
...
判据: **每一个原始顶点到简化后表面的距离, 最大值不许超过简化器报出来的
误差**。

它为什么最要紧: LOD 选择靠把这个误差投到屏幕上、与阈值比, 来决定画哪一层。
误差报小了, 选择就会在该换层的时候不换 —— 而相邻两块因此选了不同层、边界
对不上, 那就是裂缝。误差是不是上界这件事完全不必看画面, 它是一句能证伪
的话。

三版都被它当场顶掉:

**一、拿二次误差开方当误差。** 二次误差是"到一族平面的加权距离平方和",
面积加权之后量纲是 距离²×面积 —— 开方出来根本不是一个距离。

    球体简化一半   实际偏差 0.0129   报出去 0.000656   差二十倍, 报**小**

二次误差是个好的**排序**启发式 (它确实知道哪条边坍缩起来更不亏), 但它不是
界。界要单独记。

**二、拿"顶点滑了多远"当误差。** 记每个原始点到它最终落进的那个顶点的距离,
取最大。这确实是个界 —— 但量错了东西。

    平面简化一半   实际偏差 0   报出去 1.92 (整块平面的尺度)   松三千万倍

顶点沿着平面滑得很远, 而**面本身分毫未变**。LOD 要的是"表面偏了多少", 不是
"顶点滑了多远"。

**三、只在局部三角形里量距离。** 方向对了 —— 平面回到 0, 球体紧到 1.2 倍。
但点会沿着表面滑出代表顶点那一圈三角形, 于是量到的是"到一条边的距离"而不是
"到脚下那块面的距离"。

    平面简化一半   实际 0.000003   一环报 0.19   两环报 0.12

一环、两环都堵不住 —— 平坦的地方点能滑任意远。

**最后**: 老老实实全局扫, 每个原始点对全部存活三角形取最近。三个测试网格上
都是精确值 (1.00 倍)。简化器是离线工具, 而且第三天起它在 meshlet 组 (几千个
三角形) 上跑而不是整张网格 —— 拿一个空间结构换这点时间, 换来的是一处可能
出错而判据看不见的地方, 不划算。

### 判据顶出来的真缺陷: 鳍

球体简化到一成, 二十一个三角形的三个顶点**全落在接缝那条经线上** (z 恰好
为 0) —— 整片立在子午面里, 与脚下表面的法线点积在 -0.03 到 0 之间。那不是
翻过来了, 是立起来了: 一片鳍。

逐步的翻转检查 (这一次坍缩前后法线不许掉头) 拦不住它: 每一步只转几度, 点积
始终为正, 几十步下来就立起来了。

修法是给每个三角形记下**最初**的法线当锚, 离它超过 60 度就拒绝坍缩。60 而
不是 90: 卡在 90 度时三角形转到 89 度仍然放行, 而那时它已经基本立着了。

### 又一条不会红的分支

"面积塌成零就拒绝"被纵横比检查完全盖住 —— 面积为零时纵横比也是零, 而零一定
小于阈值也一定小于坍缩前的值。变异验证证实: 把它删掉, 判据毫无反应。删了。
...
最后一条的逃逸原因是量出来的: 它被"锚住最初法线"盖住了大半 —— 两者都要求
离最初朝向不超过 60 度, 于是坍缩前后最多差 120 度, 单步掉头 (180 度) 的空间
很窄。留着它不是因为它现在有用武之地, 是因为将来若为了简化率放宽那个余量
(比如放到 85 度), 单步掉头的空间立刻打开, 而那时它是唯一的拦截。

### 记下来的一件事

"真的简化了"这条判据看着最不起眼, 却是其余全部判据的前提: 一个**原样返回**
的实现在"误差是上界""不退化""流形保持""确定性"上全是满分 —— 误差 0 是任何
偏差的上界, 体积分毫不差, 流形当然保持。

这是第六周期那条"一条不会红的分支就是没有判据的分支"的另一面: **一组判据
如果对'什么都不做'全都满分, 那它们验的不是这件事**。


## 附: BC7 编码器

`lat` 补上 BC7 (2100 余行, 零第三方依赖 —— 打开 block_compression 的 bc7
feature 会把 wgpu 拉进来, 而这个工具的价值就在于离线无 GPU)。

编码器**八种 mode 全产出**, 解码器同样八种全支持。解码器完整实现是刻意
的: 它是拿来验编码器的 oracle, 只实现用到的那几种会让"编码器写错、解码器
跟着错"互相掩盖。

一开始只产出 4 种 (6/1/3/7), 第六周期收尾时补齐了剩下四种。补之前先量了
值不值 —— Sponza 三张 1024x1024, 每张 65536 个块, 逐块对比两套 mode 集合:
...
结论分得很开: **mode 4/5 (旋转 + 两套独立索引) 是这次补齐的全部价值**,
而 mode 0/2 (三子集) 在真实贴图上几乎不被选中。代价也量了: 单张烘焙从
0.48 秒涨到 1.02 秒, 其中三子集那一路占了近一半, 换来不到 1.5% 的块。
两句都写进了源码的文件头 —— 要为烘焙速度动刀时, 第一刀该落在哪里是有数
的, 不必再量一遍。

判据是"每种新 mode 都要在为它设计的内容上**被选中**"加上"编码器产出的每
一种 mode 都能被解码器读回来, 且解码后的误差与编码时算的逐位一致"。判据
的内容挑了三轮, 每一轮都被自己的失败逼回去重挑 —— 三块纯色双子集就能编到
零误差 (三子集毫无必要)、16 级灰阶反而让 mode 6 占优 (验的是别的东西)、
奇偶按列分而分区表里正好有能按列切开的图案 (一刀下去零误差)。变异 9/9。
