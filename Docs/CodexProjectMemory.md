...
老老实实记下来。
    ...
    grid 127 (16130 个物体)   每次都在第三帧 vkQueueSubmit 返回 -4
                              (VK_ERROR_DEVICE_LOST), 六十帧一帧都跑不完

已经排除的:

    不是本周期改出来的   把今天的改动逐个撤掉, 照样复现
    不是实例容量         上限从 16384 翻到 32768, 照样复现
    不是 GPU 驱动剔除    关掉它 (--no-cull) 照样复现; 而不开 meshlet 路径
                        时同样规模一切正常
    不是网格着色器       回退路径 (计算展开 + 经典顶点着色器) 一样复现
    不是 meshlet 剔除    把两级分派**都**跳过, 仍然复现
    不是奇偶             grid 39 / 41 / 101 都正常
    不是偶发             126 跑十次全过, 127 跑两次全挂

也就是说: 剩下的嫌疑落在剔除通道里那些**非分派**的部分 —— 上传、复位拷贝、
回读拷贝、兜底金字塔的布局转换 —— 或者更外围的地方。没有验证层报错先于
那次提交, 所以不是 API 误用被层抓到的那一类。

判据跑在支持的规模内 (verify.ps1 用 grid 40 与 grid 100)。这个缺陷排在下一
个工作日的第一位, 不假装它被处理过了。
