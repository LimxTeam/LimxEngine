...
老老实实记下来。第十五天又查了半天, 把范围收窄了很多, 但没查到底。

    grid 126 (15877 个物体)   十次十次地跑, 一次都没出过问题
    grid 127 (16130 个物体)   每次都在第三帧 vkQueueSubmit 返回 -4
                              (VK_ERROR_DEVICE_LOST), 一帧都跑不完

**已经排除的:**

    不是本周期改出来的   把今天的改动逐个撤掉, 照样复现
    不是实例容量         上限从 16384 翻到 32768, 照样复现
    不是显存压力         顶点池从 400 万砍到 50 万 (省 252 MB), 照样复现
    不是 GPU 驱动剔除    关掉它照样复现; 而不开 meshlet 路径时同样规模正常
    不是网格着色器       回退路径 (计算展开 + 经典顶点着色器) 一样复现
    不是奇偶             grid 39 / 41 / 101 都正常
    不是偶发             126 跑十次全过, 127 跑两次全挂

**已经定位到的:**

把 FMeshletCullPass::Execute 从头到尾逐段加提前返回, 二分下来:

    整个 Execute 空转                     不复现
    只做场景重建                          不复现
    重建 + BuildInstances (不上传)         不复现
    重建 + BuildInstances + **上传**       复现

再往里分:

    只 map / unmap, 一个字节都不写         不复现
    写实例表 (球表跳过)                    复现
    实例表只写前 15877 条 (球表照写)        复现

也就是说: **杀掉它的是那两次 CPU 侧的写, 而不是任何 GPU 工作。** 这一段
里一个分派都没有, 一次绘制都没有, 而且写的字节数与 grid 126 那次一模一样
(15877 条) 时照样挂。

而且它**与视角有关**: 相机转开去看天 (yaw 0, pitch 0.5) 时 grid 127 一切
正常; 转回网格 (pitch -0.2 或 -0.45) 就挂。

映射指针本身查过了 —— 分配器给的是块基址加子分配偏移 (FVulkanMemoryAllocator
第 591 行与第 657 行), 不是块基址。写入范围也在容量内 (16130 条 / 上限
16384 条; 64520 个浮点 / 上限 65536 个)。

**GPU 辅助验证是干净的。** 开着
VK_VALIDATION_FEATURE_ENABLE_GPU_ASSISTED_EXT 跑 grid 127, 直到设备丢失为止
一条越界访问都没报 —— 着色器没有越界读写。这与"杀掉它的是 CPU 侧的写"那条
是一致的。

**还没查的方向:** 提交之前没有任何验证层报错 (核心检查与 GPU 辅助验证都
干净), 说明不是被层抓得到的那一类。再往下要么上 VK_EXT_device_fault 问驱动
故障地址, 要么直接抓帧。两样都不是几分钟的事, 所以这一天没做完。

判据跑在支持的规模内 (verify.ps1 用 grid 40 与 grid 100)。**这条缺陷是下一
个周期的第一件事**, 不假装它被处理过了。
...
**验证**: verify.ps1 64 步全绿。第六周期新增的每一天都做了变异验证:
...
一万六千个物体上 meshlet 路径必然设备丢失 (grid 126 正常, grid 127 每次都挂)。
已经收窄到"杀掉它的是剔除通道里那两次 CPU 侧的上传写, 而不是任何 GPU 工作",
六条结构性猜想都排除了, 核心验证层与 GPU 辅助验证都干净。没查到底, 细节记在
Day 14 那一节里, 排下一个周期的第一件事。

判据跑在支持的规模内。**不假装它被处理过了。**
